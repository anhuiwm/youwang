...
public partial class SoulGuideBookList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                ...
                //int SoulType = System.Convert.ToUInt16(Request["soultype"]);
                ...
                uint Class = System.Convert.ToUInt16(jparse["class"].GetValue().ToString());
                //파라미터 값 확인 로그
                string reqval = "aid=" + Request["aid"] + "&class=" + Class;
                string reqURL = System.Convert.ToString(Request.Url);

                //PVE 시작 관련
                JsonArrayCollection MakeJson = new JsonArrayCollection("soullist");
                DB_sharding.Open();
                var command = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = DB_sharding, CommandText = "GetSoulGuideBookList" };
                command.Parameters.Add("@AID", SqlDbType.BigInt).Value = AID;
                //command.Parameters.Add("@SOULTYPE", SqlDbType.TinyInt).Value = SoulType;
                command.Parameters.Add("@CLASS", SqlDbType.TinyInt).Value = Class;
