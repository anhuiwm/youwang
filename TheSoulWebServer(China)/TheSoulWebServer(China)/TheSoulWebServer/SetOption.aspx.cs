...
public partial class SetOption : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                ...
                string CountryCode = System.Convert.ToString(jparse["countrycode"].GetValue().ToString());

                //파라미터 값 확인 로그
                string reqval = "aid=" + Request["aid"] + "&CountryCode=" + CountryCode + "&oldtime=" + retMyUpdateTime;
                string reqURL = System.Convert.ToString(Request.Url);

                //ACCOUNT 생성
                DB_sharding.Open();
                var accountcommand = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = DB_sharding, CommandText = "SetOption" };
                var outputResult = new SqlParameter("@Result", SqlDbType.Int) { Direction = ParameterDirection.Output };
                accountcommand.Parameters.Add("@AID", SqlDbType.BigInt).Value = AID;
                accountcommand.Parameters.Add("@CountryCode", SqlDbType.Char, 2).Value = CountryCode;
                accountcommand.Parameters.Add(outputResult);
                accountcommand.ExecuteNonQuery();
                int resultcode = int.Parse(outputResult.Value.ToString());
                DB_sharding.Close();
