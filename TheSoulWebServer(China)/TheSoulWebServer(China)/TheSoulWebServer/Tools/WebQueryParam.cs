...
namespace TheSoulWebServer.Tools
{
    ...
    public class WebQueryParam
    {
        ...
        public Log_Define.eLogLevel bDBLogLevel = Log_Define.eLogLevel.Default;

        private void DetailLogWriteToDB(ref TxnBlock TB, string retJson, string LogDB = DataManager_Define.LogDB)
        {
#if DEBUG
            if (bDBLogLevel == Log_Define.eLogLevel.Default)
                bDBLogLevel = Log_Define.eLogLevel.Full;
            bDBLog = SetShowLogMode = true;
#else
            if (bDBLogLevel == Log_Define.eLogLevel.Default)
            {
                //if(TheSoulDBcon.service_area == DataManager_Define.eCountryCode.Korea)
                //    bDBLogLevel = Log_Define.eLogLevel.Full;
                //else
                bDBLogLevel = setDebug ? Log_Define.eLogLevel.Full : Log_Define.eLogLevel.Simple;
            }
#endif
            //if (setDebug)
            //    bDBLogLevel = Log_Define.eLogLevel.Full;
            if ((bDBLog && SetShowLogMode && bDBLogLevel > Log_Define.eLogLevel.None) || bDBLogLevel > Log_Define.eLogLevel.Detail)
            {
                //if (requestAID > 0)
                if (!string.IsNullOrEmpty(operation))
                {
                    ...
                    if (!string.IsNullOrEmpty(TB.GetLogData("hack_skill")))
                        errCode = Result_Define.eResult.System_Hack_Detected;
                    ...
                    string query = string.Format(@"INSERT INTO {0} (AID, CID, ErrorCode, RequestURL, Operation, RequestParams, ResponseResult, BaseJson, DetailDBLog, requesttime) VALUES ( @aid, @cid, @errcode, @url, @op, @params, @response, @baseJson, @DBLog, @reqTime)",
                        SnailLogManager.SnailLogTable(ref TB, DataManager_Define.LogTableName)
                        );
                    ...
                    cmd.Parameters.AddWithValue("@baseJson", bDBLogLevel > Log_Define.eLogLevel.Simple ? returnJson : "");
                    cmd.Parameters.AddWithValue("@DBLog", bDBLogLevel > Log_Define.eLogLevel.Detail ? DBLog : "");
        ...
        public void GMToolLogToDB(ref TxnBlock TB, string gmID, string dbkey)
        {
            if (ReqParams.Count > 0)
            {
                ...
                DateTime ReqTime = System.Web.HttpContext.Current.Request.RequestContext.HttpContext.Timestamp;
                string DBLog = mJsonSerializer.ToJsonString(detailDBLog);
                string query = string.Format(@"INSERT INTO {0} (gmid, ErrorCode, RequestURL, Params, DetailDBLog, regdate) VALUES ( @gmID, @errcode, @url, @params, @DBLog, @reqTime)", GMLogTable);
                SqlCommand cmd = new SqlCommand();
                cmd.CommandText = query;
                ...
                cmd.Parameters.AddWithValue("@reqTime", ReqTime.ToString("yyyy-MM-dd HH:mm:ss.fff"));
                string query1 = string.Format(@"INSERT INTO {0} (gmid, ErrorCode, RequestURL, Params, DetailDBLog, regdate) VALUES ( '', 0, 'test', '', '', getdate())", GMLogTable);
        ...
        public long RequestAID
        {
            set { requestAID = value; }
        }
        ...
        {
            return Render(json, errorCode, true);
        }

        public string Render(string json, Result_Define.eResult errorCode, bool bEncrypt)
        {
            if (renderFlag)
                return "";
            ...
            try
            {
                ...
                json = bEncrypt ? EncryptParam(errorCode, json) : json;
                System.Web.HttpContext.Current.Response.Write(json);
        ...
        public string Render(JsonObject json, Result_Define.eResult errorCode, bool bEncrypt)
        {
            if (renderFlag)
                return "";
            ...
            try
            {
                ...
                string jsonbody = bEncrypt ? EncryptParam(errorCode, json.ToJson()) : json.ToJson();
                System.Web.HttpContext.Current.Response.Write(jsonbody);
                if (!requestUrl.Contains("RequestPrivateServer"))
                    AddReRequestInfo();
                renderFlag = true;
                return jsonbody;
        ...
        public string GM_Render(string json, Result_Define.eResult errorCode, bool bEncrypt)
        {
            if (renderFlag)
                return "";
        ...
        public string GM_Render(string json, Result_Define.eResult errorCode)
        {
            return GM_Render("", errorCode, true);
        }

        public string GM_Render(Result_Define.eResult errorCode)
        {
            return GM_Render("", errorCode, true);
        }
        ...
        public string EncryptParam(Result_Define.eResult errorCode, string json)
        {
            ...
            errCode = errorCode;
            setJson = mJsonSerializer.AddJson(setJson, DefineError.reqOperation, operation);
            setJson = mJsonSerializer.AddJson(setJson, DefineError.retResultCode, ((int)errorCode).ToString());
            ...
            if (!setDebug)
            {
                render_compression = returnJson.Length > DataManager_Define.CompressionLength;
                string EncryptString = "";
                if (render_compression)
                    TheSoulEncrypt.CompressionEncrypt(setJson.ToJson(), userEncryptKey.EncryptKey, ref EncryptString);
                else
                    TheSoulEncrypt.EncryptData(setJson.ToJson(), userEncryptKey.EncryptKey, ref EncryptString);

                setJson = mJsonSerializer.AddJson(new JsonObject(), DefineError.retEncryptData, EncryptString);
            }

            setJson = mJsonSerializer.AddJson(setJson, DefineError.retResult, (!setDebug && render_compression) ? "1" : "0");
        ...
        public void SnailLogWrite(ref TxnBlock TB)
        {
            ...
            if (setAID > 0)
            {
                ...
                if (TB.SystemLogData.ContainsKey(SnailLog_Define.SetLogKey[SnailLog_Define.SnailLogKey.renew_role_log]))
                {
                    SnailLogManager.SnailLog_renew_role_log(ref TB, setAID);
                }
                ...
                if (TB.SystemLogData.ContainsKey(SnailLog_Define.SetLogKey[SnailLog_Define.SnailLogKey.mseed_item_log]))
                {
                    string setItemLogJson = TB.GetLogData(SnailLog_Define.SetLogKey[SnailLog_Define.SnailLogKey.item_log_list]);
                    List<ItemLogInfo> setLog = mJsonSerializer.JsonToObject<List<ItemLogInfo>>(setItemLogJson);
        ...
        public void ErrorLogWrite(string jsonObj, ref TxnBlock TB)
        {
            try
            {
                ...
                if (render_errorFlag)       // check/set snail log
                {
                    SnailLogWrite(ref TB);
                }
        ...
        public void TxnBlockInit(ref TxnBlock TB, ref long retAID, bool useBaseEncrypt = false)
        {
            ...
            bDBLog = TheSoulDBcon.GetInstance().TheSoulDBInitFromGlobal(ref TB);
            ...
            string ipfirst = ipaddress.Split('.')[0];
            ...
            bool bIPPass = (ipfirst.Equals("10") || ipfirst.Equals("127") || ipfirst.Equals("172") || ipfirst.Equals("192"))
                || TheSoulDBcon.Snail_ips.FindAll(ipaddr => ipaddr.ip_address.Equals(ipaddress)).Count > 0
                ;

            //if (TheSoulDBcon.service_area != DataManager_Define.eCountryCode.China && bIPPass)
            if (bIPPass)
            {
                if (System.Web.HttpContext.Current.Request.Params.AllKeys.Contains("Debug"))
                    SetDebugMode = true;
            ...
            if (bIPPass && !operation.Equals("/RequestPrivateServer.aspx"))
                bDBLogLevel = Log_Define.eLogLevel.Full;

            if (retAID > 0)
            {
                User_Admin_LogLevel setLogLevel = TheSoul.DataManager.LogManager.GetAdminLogLevel(ref TB, retAID);
                if (setLogLevel.log_level > 1)
                    bDBLogLevel = (Log_Define.eLogLevel)setLogLevel.log_level;
            }

            userEncryptKey = useBaseEncrypt ? new User_Encrypt(TheSoulEncrypt.base64_key) : EncryptManager.GetUser_Encrypt(ref TB, retAID);
            ParamDecrypt(userEncryptKey.EncryptKey);

            if (bDBLog)
            {
                TB.Elog = DBLog;
                if (bDBLogLevel == Log_Define.eLogLevel.Full)
            ...
            if (operation != null && SnailLog_Define.GetOperationSID.ContainsKey(operation))
                TB.SetLogData(SnailLog_Define.SetLogKey[SnailLog_Define.SnailLogKey.s_event_id], SnailLog_Define.Operation_To_S_Event_ID(SnailLog_Define.GetOperationSID[operation]));                
