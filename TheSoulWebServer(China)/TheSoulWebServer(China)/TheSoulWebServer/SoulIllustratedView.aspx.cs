...
public partial class SoulIllustratedView : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                ...
                uint SoulGroup = System.Convert.ToUInt32(jparse["soulgroup"].GetValue().ToString());
                //파라미터 값 확인 로그
                string reqval = "aid=" + Request["aid"] + "&soulgroup=" + SoulGroup;
                string reqURL = System.Convert.ToString(Request.Url);

                //갱신 혼 리스트
                JsonArrayCollection MakeJson = new JsonArrayCollection("soullist");
                DB_sharding.Open();
                var command7 = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = DB_sharding, CommandText = "GetSoulIllustratedList" };
                command7.Parameters.Add("@AID", SqlDbType.BigInt).Value = AID;
                command7.Parameters.Add("@SOULGROUP", SqlDbType.Int).Value = SoulGroup;
                SqlDataReader reader = command7.ExecuteReader();
                while (reader.Read())
                {
                    JsonObjectCollection items = new JsonObjectCollection();
                    items.Add(new JsonNumericValue("soulid", System.Convert.ToUInt32(reader["SoulID"])));
                    items.Add(new JsonNumericValue("grade", System.Convert.ToUInt16(reader["Grade"])));
                    items.Add(new JsonStringValue("rewardyn", System.Convert.ToString(reader["RewardYN"])));
                    MakeJson.Add(items);
                }
                DB_sharding.Close();
            ...
            catch (Exception ex)
            {
                ...
                res.Add(new JsonStringValue("message1", ex.Message));
                res.Add(new JsonStringValue("message2", ex.ToString()));
                string json_text = res.ToString();
                var EncryptString = "";
                TheSoulEncrypt.EncryptData(0, json_text, "0", ref EncryptString);
                string JsonConvertData = "";
