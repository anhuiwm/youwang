...
public partial class FacebookFriendsSendKey : System.Web.UI.Page
{
        ...
        else
        {
            try
            {
                ...
                foreach (JsonObject item in gamefriends)
                {
                    ...
                    //친구 키 보낸 시간 체크 및 갱신
                    DB_sharding.Open();
                    var command = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = DB_sharding, CommandText = "FacebookFriendsSendKeyCheck" };
                    var outputSendCheck = new SqlParameter("@RESULT", SqlDbType.Int) { Direction = ParameterDirection.Output };
                    ...
                    command.Parameters.Add(outputSendCheck);
                    command.ExecuteNonQuery();
                    int retSendCheck = int.Parse(outputSendCheck.Value.ToString());
                    DB_sharding.Close();
                    ...
                    if (retSendCheck == 0)
                    {
                        ...
                        var outputDB_INDEXFriend = new SqlParameter("@RetDB_INDEX", SqlDbType.Int) { Direction = ParameterDirection.Output };
                        var outputRetUserNameFriend = new SqlParameter("@RetUserName", SqlDbType.NVarChar, 32) { Direction = ParameterDirection.Output };
                        var outputResultAccountFriend = new SqlParameter("@Result", SqlDbType.Int) { Direction = ParameterDirection.Output };
                        friendcommand.Parameters.Add("@AID", SqlDbType.BigInt).Value = System.Convert.ToInt64(GameFriend["friendaid"].GetValue().ToString());
                        friendcommand.Parameters.Add(outputDB_INDEXFriend);
                        friendcommand.Parameters.Add(outputRetUserNameFriend);
                        friendcommand.Parameters.Add(outputResultAccountFriend);
                        friendcommand.ExecuteNonQuery();
                        int retDB_INDEXFriend = int.Parse(outputDB_INDEXFriend.Value.ToString());
                        string retMyNameFriend = outputRetUserNameFriend.Value.ToString();
                        int retResultAccountFriend = int.Parse(outputResultAccountFriend.Value.ToString());
                        DB_common.Close();

                        //샤딩 DB 만큼 루프 돌리기
                        int DB_INDEX = System.Convert.ToInt32(retDB_INDEXFriend);
                        ...
                        var command2 = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = DB_sharding_var, CommandText = "FacebookFriendsSendKey" };
                        command2.Parameters.Add("@AID", SqlDbType.BigInt).Value = AID;
                        command2.Parameters.Add("@FRIENDAID", SqlDbType.BigInt).Value = System.Convert.ToInt64(GameFriend["friendaid"].GetValue().ToString());
                        command2.Parameters.Add("@USERNAME", SqlDbType.NVarChar, 32).Value = retMyName;
                ...
                //우정포인트 체크
                DB_sharding.Open();
                var command4 = new SqlCommand { CommandType = CommandType.StoredProcedure, Connection = DB_sharding, CommandText = "FacebookFriendlyPointCheck" };
